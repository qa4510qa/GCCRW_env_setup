...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.publish("MCU_00/test", "reconnect successfully!");
      // ... and resubscribe
...
/*
void publish_data(char topic[],char description[],char data[]) {
  msgStr=strcat(description,data);
...
//  msgStr.toCharArray(msg, arrSize); // 把String字串轉換成字元陣列格式
  client.publish(topic, data);       // Publishing
